...
class Client(object):
    ...
    def __init__(self, conf, parsed_url):
        self.conf = conf
        self.conf.register_opts(monclient_opts, group='monasca')
        for group, options in opts.list_keystoneauth_opts():
            conf.register_opts(list(options),
                               group=None if group == "DEFAULT" else group)

        self._retry_interval = self.conf.database.retry_interval * 1000
