...
class Connection(base.Connection):
    ...
    def __init__(self, conf, url):
        ...
        self.conf.register_opts(OPTS, group='monasca')
    ...
    def get_resources(self, user=None, project=None, source=None,
        ...
        if not start_timestamp:
            time_lenght = datetime.timedelta(seconds=600)
            start_timestamp = timeutils.utcnow() - time_lenght
    ...
    def get_samples(self, sample_filter, limit=None):
        ...
        if not sample_filter.start_timestamp:
            time_lenght = datetime.timedelta(seconds=600)
            sample_filter.start_timestamp = timeutils.utcnow() - time_lenght
        ...
        if sample_filter.metaquery:
            _dimensions.update(sample_filter.metaquery)
    ...
    def get_meter_statistics(self, filter, period=None, groupby=None,
        ...
        if not filter.start_timestamp:
            time_lenght = datetime.timedelta(seconds=600)
            filter.start_timestamp = timeutils.utcnow() - time_lenght
        ...
        if filter.metaquery:
            dims_filter.update(filter.metaquery)
        dims_filter = {k: v for k, v in dims_filter.items() if v is not None}
    ...
    def _ensure_start_time_le_end_time(self, start_time, end_time):
        if start_time is None:
            time_lenght = datetime.timedelta(seconds=600)
            start_time = timeutils.utcnow() - time_lenght
        if end_time is None:
