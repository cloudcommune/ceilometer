...
class _Base(plugin_base.PollsterBase):
    ...
    def make_samples(self, ori_samples):
        for data in ori_samples or []:
            if data is None:
                continue
            ...
            volume = data[0]['value']
            resource_id = data[0]['dimensions']['resourceId']
            project_id = data[0]['dimensions'].get('projectId')
            customer_id = data[0]['dimensions'].get('userId')
            resource_metadata = data[0]['dimensions']
            yield sample.Sample(
    ...
    def get_samples(self, manager, cache, resources):
        ...
        for resource in resources:
            ...
            sample_data = ext.get_sample_data(self.meter_name,
                                              ...
                                              params,
                                              cache)
            for each_sample in self.make_samples(sample_data):
                yield each_sample


class OSSReceiveSizesPollster(_Base):
    meter_name = "bucket.standard.storage.size2"
    meter_type = sample.TYPE_CUMULATIVE
    meter_unit = 'B'


class OSSFeeSizesPollster(_Base):
    meter_name = "bucket.fee.standard.storage.size2"
    meter_type = sample.TYPE_CUMULATIVE
    meter_unit = 'B'
