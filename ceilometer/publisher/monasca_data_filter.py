...
class MonascaDataFilter(object):
    ...
    def process_sample_for_monasca(self, sample_obj):
        ...
        if sample_meta:
            ...
            # improve_metadata
            for meta_key in self._mapping['improve_metadata']['common']:
                monasca_key, val = self._get_value_metadata_for_key(
                    sample_meta, meta_key)
                dimensions['metadata.' + monasca_key] = val

            if meter_name in self._mapping['improve_metadata'].keys():
                for meta_key in self._mapping['improve_metadata'][meter_name]:
                    monasca_key, val = self._get_value_metadata_for_key(
                        sample_meta, meta_key)
                    if val != '':
                        dimensions['metadata.' + monasca_key] = val
